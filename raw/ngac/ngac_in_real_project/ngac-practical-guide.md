...
| User             | U        | Người dùng cụ thể       | alice, charlie      |
| Object Attribute | OA       | Nhóm tài nguyên         | DeptA_Drive, Ch_deptA_Content |
...
#### Ví dụ cụ thể: alice download file từ Drive phòng DeptA

```
1. Frontend: GET /drive/items/{file-id}/download
2. Backend:  SQL → lấy file info, bao gồm ngac_node_id = "oa-deptA-drive" (OA folder cha)
3. NGAC:     checkAccess("ngac-alice", "oa-deptA-drive", "read")
             → alice ∈ DeptA_Dept → DeptA_Dept ══[read]══→ DeptA_Resources
...
Upload file "baocao.pdf" vào folder DeptA_Drive:
  ① checkAccess(alice, DeptA_Drive_OA, "write") → ✅
  ② INSERT INTO drive_items (..., ngac_node_id = "oa-deptA-drive") ← chỉ SQL
...
│   ├── Phòng Nội Vụ       → hr_mgr (TP), hr_staff1, hr_staff2
│   ├── Phòng Kinh Doanh
│   │   ├── Team Dự Án      → proj_lead (TL), proj_mem1
│   │   ├── Team Marketing   → mkt_lead (TL), mkt_mem1
│   │   └── Team Account Mgr → acc_lead (TL), acc_mem1
│   ├── Phòng AI            → ai_mgr (TP), ai_dev1, ai_dev2
│   ├── Phòng Hạ Tầng
│   │   ├── Devops           → devops_lead (TL), devops_eng1
│   │   └── Helpdesk         → helpdesk_lead (TL), helpdesk_eng1
│   └── Phòng DeptA
│       ├── Team Appserver 1 → charlie (TL), dave
│       ├── Team Appserver 2 → eve (TL), frank
│       ├── Team Appserver 3 → grace (TL), heidi
│       └── Team BO          → alice (TL), bob
│       └── Chung: dev_alice (Dev), vp_bob (PP)
│
├── Miền Bắc (UA: MienBac_Region)
│   ├── Phòng Nội Vụ        → mb_hr_mgr (TP), mb_hr_staff1
│   ├── Phòng Kinh Doanh    → mb_biz_mgr (TP), mb_biz_staff1, mb_biz_staff2
│   ├── Phòng Hạ Tầng       → mb_it_mgr (TP), mb_it_staff1
│   └── Phòng DeptA
│       ├── Team Appserver 4 → mb_app4_lead (TL), mb_app4_dev1
│       └── Team BO Bắc      → mb_bo_lead (TL), mb_bo_dev1
...
```mermaid
graph TD
    subgraph deptA["Phòng DeptA — Miền Nam"]
        DEPT["👥 DeptA_Dept"]
        CHIEF["👥 DeptA_Chief"]
        RES["📁 DeptA_Resources"]
        DRIVE["📁 DeptA_Drive"]
        APPR["📁 DeptA_ApprovalScope"]
        CH_C["📁 Ch_deptA_Content"]
        CH_M["👥 Ch_deptA_Members"]
    ...
    vp_bob["👤 vp_bob PP"] -->|asg| CHIEF
    charlie["👤 charlie TL"] -->|asg| T1L
    dev_alice["👤 dev_alice Dev"] -->|asg| DEPT
    dave["👤 dave Dev"] -->|asg| T1
    eve["👤 eve TL"] -->|asg| T2
    grace["👤 grace TL"] -->|asg| T3
    alice["👤 alice TL"] -->|asg| TBO
...
**Scenario**: `alice` share file "BáoCáoQ1.pdf" (nằm trong folder "Reports" của phòng DeptA) cho `bob` (SuperB).
...
├────────────┼────────────────┼──────────┼──────────────────┼─────────────┤
│ folder-001 │ Reports        │ folder   │ oa-reports-deptA  │ root-deptA   │
│ file-001   │ BáoCáoQ1.pdf   │ file     │ oa-reports-deptA  │ folder-001  │
│            │                │          │ ↑ kế thừa OA     │             │
└────────────┴────────────────┴──────────┴──────────────────┴─────────────┘

NGAC Graph (chỉ có nodes sau):
  oa-reports-deptA (OA) → oa-deptA-drive → oa-deptA-resources → PC_KyLan
```

> ⚠️ File `file-001` KHÔNG có NGAC node riêng. Nó dùng `oa-reports-deptA` (OA của folder cha) để checkAccess.

#### Bước 2 — Thực hiện share (5 thao tác)

```
① CreateNode(OA): "Share_BáoCáoQ1_abc123"       → ngac_nodes
② Assignment: oa-reports-deptA → Share_OA          → ngac_assignments
③ Assignment: Share_OA → PC_Global                → ngac_assignments
④ Association: ngac-bob → Share_OA [read]     → ngac_associations
...
graph TD
    ...
    bob["👤 bob<br>SuperB"] -.->|"④ 🔑 [read]"| SHARE
    FILE["📄 BáoCáoQ1.pdf<br>SQL only"] -.->|"ngac_node_id"| REPORTS

    style SHARE fill:#e67e22,color:#fff,stroke:#e74c3c,stroke-width:3px
    style bob fill:#e74c3c,color:#fff
    style FILE fill:#bdc3c7,color:#333,stroke-dasharray: 5 5
```

### 5.3 CheckAccess — 3 loại user

#### Case A: Internal user (charlie — cùng DeptA)

```
checkAccess("ngac-charlie", "oa-reports-deptA", "read")

Traversal:
  charlie → AppSrv1_Team → DeptA_Dept ──[read,write,upload]──→ DeptA_Resources
  DeptA_Resources ← DeptA_Drive ← oa-reports-deptA ✅

→ ALLOW (đường workspace bình thường, KHÔNG đi qua Share_OA)
```

#### Case B: External user (bob — SuperB, có share)

```
checkAccess("ngac-bob", "oa-reports-deptA", "read")

Traversal:
  ❌ Đường KyLan: bob KHÔNG thuộc UA nào của KyLan → DENY
  ✅ Đường share: bob ──[read]──→ Share_OA ← oa-reports-deptA ✅

→ ALLOW (vì có association trực tiếp đến Share_OA)
```

#### Case C: Random user (ai_mgr — Phòng AI, không share)

```
checkAccess("ngac-ai_mgr", "oa-reports-deptA", "read")

Traversal:
  ❌ Đường DeptA: ai_mgr thuộc AI_Dept, KHÔNG thuộc DeptA_Dept → ko reach DeptA_Resources
  ❌ Đường share: ai_mgr KHÔNG có association đến Share_OA
...
**Scenario**: Thay vì share cho bob, alice share public "BáoCáoQ1.pdf".
...
| **④ Association**      | `bob → Share_OA` | **`PublicUsers → Share_OA`** |
...
```
checkAccess("ngac-any-user", "oa-reports-deptA", "read")

Traversal:
  any-user → PublicUsers (UA) ──[read]──→ Share_OA ← oa-reports-deptA ✅
...
```
Share_OA gán vào oa-reports-deptA (folder OA)
→ TẤT CẢ files bên trong folder đều kế thừa quyền
→ Vì files dùng ngac_node_id = oa-reports-deptA (OA của folder cha)
...
```
NGAC Graph:
  oa-reports-deptA ──→ Share_OA ← bob   ← XÓA HẾT

Kết quả:
  checkAccess("ngac-bob", "oa-reports-deptA", "read") → DENY
  checkAccess("ngac-charlie", "oa-reports-deptA", "read")   → vẫn ALLOW (đường workspace)
```
...
User `bob` (SuperB) tham gia group "kylan-superb-trao-doi":
...
> → bob chỉ thấy group này, KHÔNG thấy bất kỳ thứ gì của KyLan.
...
**Ví dụ: alice submit "Deploy production":**

```sql
-- ② Resolve: NGAC query 1 lần
-- Step 1 TL: charlie (thuộc AppSrv1_Lead, có [approve])
-- Step 2 PP: vp_bob (thuộc DeptA_Chief, có [approve])

-- ③ Ghi denormalized
INSERT INTO tenant_kylan.approval_assignments VALUES
  ('aa-001', 'req-001', 1, 'ngac-charlie',      'AppSrv1_Lead', 'pending'),
  ('aa-002', 'req-001', 2, 'ngac-vp_bob',   'DeptA_Chief',   'pending');
```
...
WHERE a.user_node_id = 'ngac-charlie'
...
WHERE scope_oa_id = 'oa-deptA-approval-scope'
  AND status = 'pending'
...
`vp_bob` (Phó phòng DeptA) nghỉ hưu → `hoangttt` lên thay.
...
WHERE child_id = 'ngac-vp_bob' AND parent_id = 'ua-deptA-chief';

-- Thêm người mới
INSERT INTO ngac_assignments (id, child_id, parent_id)
VALUES ('a-new-chief', 'ngac-hoangttt', 'ua-deptA-chief');
```
...
│ ngac-vp_bob ← │ DeptA_Chief   │ pending │  ← VẪN TRỎ NGƯỜI CŨ!
```
...
```sql
-- Lịch sử CỦA vp_bob vẫn nguyên vẹn
SELECT * FROM approval_assignments
WHERE user_node_id = 'ngac-vp_bob'
  AND status IN ('approved', 'rejected');
-- → "vp_bob đã duyệt ngày X" — audit trail giữ nguyên
```
...
> **Quy ước tên**: `U_user_diana` = user Diana, `U_alice` = user Nguyễn Lê Văn Hoàng.
...
CacheInvalidator:
  ...
  2. parent = UA type → GetDescendants(UA_manager_mn) → find U nodes
     → thêm vào affectedUsers: {U_user_diana, U_alice, ...}
  3. Redis DEL:
...
> - `U_alice` = user Nguyễn Lê Văn Hoàng (nhân viên khác cùng UA)
...
```
  ...
  │                    ▲                │                                     │
  │  U_alice ──assign──┘             │  U_alice ──assign──► UA_Staff_KT│
  │                                     │                                     │
  ...
  │  → ALLOW ✅                         │  → DENY ❌                          │
  │                                     │                                     │
  │  CheckAccess(alice, OA_KT, read) │  CheckAccess(alice, OA_KT, read)│
  │  → ALLOW ✅                         │  → ALLOW ✅ (không bị ảnh hưởng)    │
  └─────────────────────────────────────┴─────────────────────────────────────┘

  Thay đổi:
    ╳  REMOVED: U_user_diana ──assign──► UA_Staff_KeToan_MN    ← bị admin xóa
    ✓  GIỮ NGUYÊN: U_alice ──assign──► UA_Staff_KeToan_MN
...
════════════════════════════════════════════════════════════════
  ...
  Cache state:
    ...
    │ U_alice    │ OA_ws_kylan_KeToan_MN     │ read  │ true     │ 42      │
    └──────────────┴────────────────────────────┴───────┴──────────┴─────────┘

  Redis:
    ngac:access:U_user_diana:OA_ws_kylan_KeToan_MN:read  → ALLOW
    ngac:access:U_alice:OA_ws_kylan_KeToan_MN:read  → ALLOW
...
════════════════════════════════════════════════════════════════
    ...
    2. incrementAndInvalidate(ctx, "U_user_diana", "UA_Staff_KeToan_MN"):
       ...
       f) cache.InvalidateForNodes("U_user_diana", "UA_Staff_KeToan_MN")
          ...
          → UA_Staff_KeToan_MN = type UA → GetDescendants()
            → tìm U nodes còn lại: {U_alice}
            → DEL ngac:access:U_alice:*                            ← xóa key Hoàng (precaution)
  ...
  Cache state sau T1:
    ...
    ⚠️  Lưu ý: Row của U_alice CŨNG bị xóa (vì InvalidateByUser xóa theo
    ...
    → Thực tế: chỉ row U_user_diana bị xóa, row U_alice vẫn còn nhưng Redis
...
════════════════════════════════════════════════════════════════
  T3: Hoàng mở app → CheckAccess("U_alice", "OA_ws_kylan_KeToan_MN", "read")
════════════════════════════════════════════════════════════════

  ReadServer.CheckAccess():
    L1: Redis → MISS (key bị DEL ở T1.f — precaution)
    L2: Materialized → HIT! (row U_alice vẫn còn, version=42 < 43 → STALE → MISS)
    L3: BFS Graph traversal:
        → U_alice ──assign──► UA_Staff_KeToan_MN ──assoc(read)──► OA
        → ALLOW (path vẫn tồn tại!)

    populateCaches():
      → L2: UPSERT (U_alice, OA_ws_kylan_KeToan_MN, read, true, 43)
      → L1: SET ngac:access:U_alice:OA_ws_kylan_KeToan_MN:read → ALLOW
...
════════════════════════════════════════════════════════════════
  ...
  Graph (sau khi xóa edge):
    ...
    U_alice ──assign──► UA_Staff_KeToan_MN ──assign──► UA_NhanVien_MN
                                   │
                                   ├──assoc(read)──► OA_ws_kylan_KeToan_MN
                                   └──assoc(write)─► OA_ws_kylan_KeToan_MN
...
| _(none)_ | Shared                      | Types, constants, helpers dùng chung                   | `models.go`                                  |
...
Shard KyLan chứa:
  ...
  │  U_alice, U_charlie, U_vp_bob, ...             │
  └────────────────────────────────────────────────────┘
  ┌─ PC_Global ────────────────────────────────────────┐
  │  UA_PublicUsers, OA_PublicDocs                      │
  │  U_alice (cũng thuộc PublicUsers)               │
  │  U_bob (external user, chỉ có ở PC_Global)    │
  └────────────────────────────────────────────────────┘
```
...
flowchart TD
    subgraph "BFS từ User"
        U["U_alice"] --> UA_DeptA["UA_DeptA_Dept"]
...
```
alice muốn read DeptA_Drive

BFS user:  alice → DeptA_Dept → MienNam → Members → PC_KyLan
                     + alice → PublicUsers → PC_Global
...
```
bob (external) muốn read Share_OA (tài liệu share từ KyLan)
...
BFS user:  bob → PublicUsers → PC_Global
           → userPCs = {PC_Global}
...
```
bob (external) muốn read OA_DeptA_Drive (internal resource)
